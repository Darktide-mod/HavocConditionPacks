...
$statePath = Safe-Path $stateRoot 'native-melee-resources-installation.json'
$legacyStatePath = Safe-Path $stateRoot 'native-melee-installation.json'
...
# Old HCM installations own an external Lua copy as well as the resources.
# Preserve their rollback record; never overwrite it with a resource-only one.
if ((Test-Path -LiteralPath $legacyStatePath) -and -not (Test-Path -LiteralPath $statePath)) {
    ...
    if ($legacyState.status -eq 'installed' -and $legacyState.game -eq $game) {
        if ($Mode -eq 'Uninstall') {
            & (Join-Path $PSScriptRoot 'legacy/Install.ps1') -GameDirectory $game -PackageRoot $PackageRoot -Mode Uninstall
            return
        }
        if ((Hash-File (Safe-Path $game 'binaries/Darktide.exe')) -ne $manifest.executable_sha256) { throw 'Game version differs from the animation resources.' }
        foreach ($patch in $manifest.patches) {
            $target = Safe-Path $game ('bundle/' + $patch.file)
            if (-not (Test-Path -LiteralPath $target) -or (Hash-File $target) -ne $patch.sha256) {
                throw 'Legacy animation resources differ. Run Uninstall-native-melee.cmd first, then install this version.'
            }
        }
        if ((Hash-Bytes ([HCMNativeMelee.Database]::Transform($databaseBytes,$keys,$false))) -ne $databaseHash) { throw 'Legacy animation registration is incomplete. Uninstall it first.' }
        Write-Output 'Matching legacy animation resources are already installed. No files changed; the original rollback record is preserved.'
        return
    }
}
...
if ($manifest.package_files.Count -gt 0 -and (Test-Path -LiteralPath $package)) {
